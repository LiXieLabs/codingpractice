...
public class LargestRectangleInHistogram {
    ...
    /**************** Solution 1: Monotonic Ascending Stack ***************/
    /**
    ...
    public int largestRectangleArea(int[] heights) {
        ...
        for (int i = 0; i <= heights.length; i++) {
            while (monoIncStack.peek() != -1 && heights[monoIncStack.peek()] >= (i == heights.length ? 0 : heights[i])) {
                int j = monoIncStack.pop(); // height
                int k = monoIncStack.peek(); // left short bar
                res = Math.max(res, (i - k - 1) * heights[j]); // i is right short bar
            }
            monoIncStack.push(i);
        }
        return res;
