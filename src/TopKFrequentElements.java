...
public class TopKFrequentElements {
     ...
     * 求 k 大值，用 minHeap
     *
     * Time: O(N) + O(NlogK) + O(KlogK) = O(NlogK)   Space: O(N) + O(K) = O(N)
     */
    ...
    private void quickSelect(Integer[] A, int start, int end, int k) {
        ...
        int pivot = counter.get(A[index]); // pivot 值必须先读取，因为会被覆盖！！！
        int left = start, right = end;
        while (left <= right) {
            while (left <= right && counter.get(A[left]) < pivot) left++; // 注意！！！此处是比较 counter value！！！
    ...
    public static void main(String[] args) {
        ...
        print(solution.topKFrequent(new int[]{1,2,1,2,1,3}, 2)); // [1,2]
        print(solution.topKFrequent(new int[]{1}, 1)); // [1]
        // [-68,73,-12,75,-55,-9,5]
